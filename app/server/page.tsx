import ServerDemoClient from "./inner";
import { preloadQuery, preloadedQueryResult } from "convex/nextjs";
import { api } from "@/convex/_generated/api";

export default async function ServerPage() {
  const preloaded = await preloadQuery(api.jobs.listActive, {
  ...
  return (
    <main className="p-8 flex flex-col gap-4 mx-auto max-w-2xl">
      ...
      <div className="flex flex-col gap-4 bg-slate-200 dark:bg-slate-800 p-4 rounded-md">
        ...
        <code>
          <pre>{JSON.stringify(data?.page ?? data, null, 2)}</pre>
        </code>
      </div>
      <ServerDemoClient preloaded={preloaded} />
    </main>
  );
}
